...
dt = 1.0
t0 = 0
tf = 480 # 15 mins #180#780 # 12 minutes # 180 # 3 minutes
RANDOM_MEAN = 0
RANDOM_STD = 0.1
global ann_list
global prev_t
    ...
    for F in freq_list:
        for i in range(0, yaw_list.shape[0]):
            ...
            if DEBUG:
                ...
                frequency = max([STATE_SHARING_FREQUENCY, CONTROL_FREQUENCY])
                dt = 1.0/frequency
                time_step_division = 10.0
                state_sharing_counter = -1
                if i == 0:
                    state_sharing_offsets = np.random.randint(low=0, high=time_step_division/STATE_SHARING_FREQUENCY, size=(NUM_DRONES)) # np.zeros(shape=NUM_DRONES)#

                t_int_0 = int(t0) + np.random.randint(low=0,high=10) # allows an offset in the state sharing updates
                t_int = t_int_0
                t = t_int / (STATE_SHARING_FREQUENCY*time_step_division)
                while t_int<int(tf*frequency*time_step_division):
                    zero_phase += SPEEDS['cruise']*dt/(time_step_division*r)
                    t_int += 1
                    t += dt/time_step_division

                    state_sharing_counter += 1
                    if state_sharing_counter == time_step_division/STATE_SHARING_FREQUENCY:
                        state_sharing_counter = 0
                    ...
                    d_angle = speeds*dt/(time_step_division*r)
                    angles += d_angle
                    angles[angles>(2*np.pi)] -= 2*np.pi

                    # check if state sharing should be published for any drone
                    # state sharing updates
                    if STATE_SHARING_FREQUENCY < CONTROL_FREQUENCY:
                        condition_statesharing = (state_sharing_counter == state_sharing_offsets).any()
                        condition_control = True
                    else:
                        condition_statesharing = True
                        condition_control = t_int%(int(STATE_SHARING_FREQUENCY*time_step_division/CONTROL_FREQUENCY)) == 0

                    if condition_statesharing:
                        shared_angles[state_sharing_offsets==state_sharing_counter] = angles[state_sharing_offsets==state_sharing_counter]

                        # update_known_angles()
                        known_angles = np.tile(shared_angles, (known_angles.shape[0],1))
                        np.fill_diagonal(known_angles, angles)
                    ...
                    if t_int%(frequency*time_step_division*10) == 0:
                        print(t,speeds)
                ...
                if VISUALIZATION:
                    ...
                    write_list_to_file(metrics_data_list, OUTPUT_PATH+method+"_with_offset_control_freq_1_" + str(F) + "_data.csv")
                    metrics_data_list = []
