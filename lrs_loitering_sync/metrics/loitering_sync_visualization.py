...
# Virtual yaw method with pulse to determine the timestamp of the starting point
# each drone will be assigned a random initial timestamp that gets incremented in the update loop
# The first drone to reach a yaw of 0 would send a pulse to save that timestamp as 0 for synchronization drone
# Assumption: Each drone is already loitering and ready to receive
# The pulse is only sent once and that is used to set the virtual yaw
def get_virtual_angle_with_pulse(t):
    global virtual_timestamps
    global virtual_zero_timestamps
    global desired_angles

    # if timestamps arent set yet, wait for a pulse
    if (virtual_zero_timestamps == 0).all():
        YAW0_RANGE = 0.3*dt
        yaws = angles.copy()
        # wait for a drone to reach angle 0 and send a pulse:
        # change all angles to -pi to pi range
        yaws = (yaws + pi) % (2 * pi) - pi
        if (abs(yaws) < YAW0_RANGE).any():
            virtual_zero_timestamps = virtual_timestamps.copy()
    else:
        loiter_time = 2*pi*r/SPEEDS['cruise']
        # calculate the yaw of the virtual drone
        time = virtual_timestamps - virtual_zero_timestamps
        desired_angles = (time % loiter_time) / loiter_time * 2 * pi
        update_set_speeds()
...
np.random.seed(0)
# random yaw placement for 100 trials # typically used
yaw_list = np.random.uniform(low=0.0, high=1.0, size=(100,NUM_DRONES)) * 360.0

# random yaw placement that shows the issue with mean. Majority will be close and minority will be close but in the opposite side
# minority_yaws = np.random.uniform(low=0.0, high=1.0, size=(100,1)) * 180.0
# majority_yaws =  np.random.uniform(low=-10.0, high=10.0, size=(100,9)) + minority_yaws + 180.0 # 20 degrees random range within random location
# yaw_list = np.concatenate((majority_yaws, minority_yaws), axis=1)

# random yaw placement that shows the worst case scenario for ideal. Fully distributed drones.
# yaw_distribution = np.arange(10) * 360.0/NUM_DRONES
# yaw_list = np.random.uniform(low=-5.0, high=5.0, size=(100,NUM_DRONES)) + yaw_distribution
...
dt = 1.0
t0 = 0
tf = 180 # 15 mins #180#780 # 12 minutes # 180 # 3 minutes
RANDOM_MEAN = 0
RANDOM_STD = 0.1
global ann_list
global prev_t
...
# variables for virtual pulse method:
global virtual_timestamps
global virtual_zero_timestamps
virtual_timestamps = np.random.randint(low=0, high=100000, size=NUM_DRONES)
virtual_zero_timestamps = np.zeros_like(virtual_timestamps)


method = 'ideal3'#'yaw_n_ramp_kuramoto' # 'ideal3'
# methods that control speed directly
speed_method_list = ['yaw0', 'yaw0_speedup', 'yaw0_ramp', 'yaw0_ramp_saturated', 'virtual_pulse', 'kuramoto', 'mean_kuramoto', 'yaw0_ramp_kuramoto', 'yaw_n_ramp', 'yaw0_ramp_random_jolt', 'yaw0_ramp_calculated_jolt', 'yaw0_ramp_kuramoto_improved', 'yaw_n_ramp_kuramoto']
METHODS = {
    ...
    'virtual_pulse': get_virtual_angle_with_pulse,
...
# function to update virtual timestamps for the virtual pulse method:
...
def update(t, speed):
        ...
        # time.sleep(dt)
        # update timestamps fo each drone
        if method == 'virtual_pulse':
            update_timestamps()
        update_known_angles()
        if method in speed_method_list:
            METHODS[method](t)
        else:
        ...
        for i in range(N):
            ...
            if VISUALIZATION:
                ...
                dx, dy = get_arrow(angles[i], speeds[i])
...
if __name__ == "__main__":
    ...
    freq_list = [1.0]#[0.05, 0.1, 0.5, 1.0]
    for F in freq_list:
        for i in range(0, yaw_list.shape[0]):
            ...
            if method == 'yaw0_ramp_kuramoto_improved':
                set_speeds = np.ones(angles.shape)*SPEEDS['high']
            else:
            ...
            if DEBUG:
                ...
                if VISUALIZATION:
                    ...
                    write_list_to_file(metrics_data_list, OUTPUT_PATH+method+"_" + str(NUM_DRONES) + "_drones_" + "data.csv")
                    metrics_data_list = []
